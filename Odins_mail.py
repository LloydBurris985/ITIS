# odin_mail.py
# Odin Mail v0.1.2 – Temporal Email Client with Production Polling
...
class UserState:
    def __init__(self, username: str):
        ...
        self.private_secret = secrets.token_bytes(32)  # shared with B
...
def send_message(user: UserState, eye: OdinsEye, msg: Dict):
    ...
    # Enforce runway end (optional hard limit)
    if coord["end_mask"] > user.runway_start + 10000:
        print("Warning: message exceeded suggested runway size")
    ...
    if msg["delivery_date"]:
        ...
        print("Sent (dropped into runway)")

    user.save()


# ────────────────────────────────────────────────
# Polling with Prefix + Length
# ────────────────────────────────────────────────

def poll_inbox(user: UserState, eye: OdinsEye):
    runway_start = user.runway_start
    runway_end = runway_start + 10000  # suggested limit
    ...
    for mask in range(current, batch_end, POLL_STEP_SIZE):
        try:
            ...
            coord_full["length_bytes"] = length_bytes + 8  # + prefix
...
def main():
    ...
    print(f"Runway: {user.runway_start} → {user.runway_start + 10000} (limited)")
