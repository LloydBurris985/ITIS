...
class OdinsEye:
    ...
    def encode(self, data: bytes) -> Dict[str, any]:
        ...
        Returns: {
            'version', 'start_mask', 'end_mask', 'anchor_mask',
            'last_choice', 'last_direction', 'length_bytes', 'original_hash'
        }
        """
        if not data:
            return {
    ...
    def decode(self, coord: Dict[str, any]) -> bytes:
        ...
        if length_bytes == 0:
            ...
            if expected_prev != anchor_mask:
                if end_mask == self.LOW:
                    ...
                    direction = -1
                if expected_prev != anchor_mask:
                    raise ValueError("Anchor mismatch – coordinate may be invalid")
            ...
            while remaining_chunks > 0:
                ...
                if not found:
                    raise ValueError("Backward decode failed – no valid previous mask")

                remaining_chunks -= 1

            bit_str = bit_buffer[::-1][:length_bytes * 8]
            byte_data = [int(bit_str[i:i+8], 2) for i in range(0, len(bit_str), 8) if len(bit_str[i:i+8]) == 8]
        ...
        if expected_hash and hashlib.sha256(recovered).hexdigest() != expected_hash:
            raise ValueError("Hash mismatch – recovered data does not match original")

        return recovered

    def decode_stream(self, coord: Dict[str, any], chunk_size: int = 1024*1024) -> Iterator[bytes]:
        """
        Streaming decoder – yields chunks for very large files (10 GB+).
        Memory efficient – does not load full data at once.
        """
        end_mask = coord["end_mask"]
        anchor_mask = coord["anchor_mask"]
        last_choice = coord["last_choice"]
        ...
        # Anchor last step (same as non-streaming)
        delta = (last_choice - self.CENTER) * self.STEP_FACTOR
        ...
        while remaining_chunks > 0:
            ...
            # Yield chunks as buffer fills
            while len(bit_buffer) >= 8 * (chunk_size // 1):
                chunk_bits = bit_buffer[:8 * (chunk_size // 1)]
                chunk_bytes = bytes(int(chunk_bits[i:i+8], 2) for i in range(0, len(chunk_bits), 8))
                yield chunk_bytes
                bytes_yielded += len(chunk_bytes)
                bit_buffer = bit_buffer[len(chunk_bits):]
        ...
        while len(bit_buffer) >= 8:
            ...
            yield bytes([int(byte_str, 2)])
            bytes_yielded += 1
            bit_buffer = bit_buffer[8:]

    def decode_to_file(self, coord: Dict[str, any], output_path: str, chunk_size: int = 1024*1024):
        """Stream decode directly to disk – ideal for large lattice sections."""
        total_written = 0
